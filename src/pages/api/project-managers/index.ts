...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getProjectManagers() {
    ...
    const response = await prisma.project_manager
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'project_manager'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createProjectManager() {
    ...
    const data = await prisma.project_manager.create({
      data: body,
    });
