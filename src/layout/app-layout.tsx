...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['project-manager', 'project-owner', 'team-leader', 'team-member'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<ProjectManagerInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/project-managers` : null),
    () => getProjectManagers({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
...

    {
      ...
      path: isTenantUser ? `/project-managers/view/${data?.[0]?.id}` : '/project-managers',
